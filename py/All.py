...
TEST_PORT = 443   # 測試端口
MAX_THREADS = 30  # 最大線程數
TOP_NODES = 100    # 顯示和保存前N個最快節點
TXT_OUTPUT_FILE = "IP.txt"    # TXT結果保存文件
...
"131.0.72.0/24"
    ...
    def save_results(self, results):
        ...
        try:
            # 只取前30名結果
            top_results = results[:300]  # 明確只取前30名
            ...
            with open(TXT_OUTPUT_FILE, 'w', encoding='utf-8') as f:
                ...
                for i, node in enumerate(top_results):
                    # 獲取IP的國家信息（已經是中文）
                    country = get_ip_country(node['ip'])
                    line = f"{node['ip']}#CF 優選IP {node['response_time_ms']}ms\n"
                    f.write(line)
