...
def parse_ips_and_speeds(html: str) -> List[Tuple[str, str, float]]:
    ...
    for table in soup.find_all("table"):
        ...
        # Map columns: look for IP and 下載速度
        ip_col = None
        speed_col = None
        for idx, h in enumerate(headers):
            ...
            if ip_col is None and ("ip" in h_lower or "地址" in h):
                ip_col = idx
            if speed_col is None and ("下載速度" in h or "speed" in h_lower):
                speed_col = idx
...
def format_output(ip: str, speed_display: str) -> str:
    print(f"{ip}#【優選 Nodes】{speed_display}")
    return f"{ip}#【優選 Nodes】{speed_display}"
...
def main() -> int:
    ...
    for url in REGION_URLS:
        try:
            ...
            if region_pairs:
                ...
                print(f"解析爲空: {url}", file=sys.stderr)
        except Exception as e:
            print(f"請求失敗: {url} -> {e}", file=sys.stderr)

    if not any_success or not all_pairs:
        print("未能解析到任何IP與速度對", file=sys.stderr)
        return 2
    ...
    try:
        ...
        print(f"寫入文件失敗: {e}", file=sys.stderr)
        return 3
